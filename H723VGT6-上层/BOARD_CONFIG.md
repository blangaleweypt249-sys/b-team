...
- 上层分别对 PE0、PD6 连续确认 3 帧后才接受新档位；普通按键、普通开关以及 `switch_bits.bit4` 的 PD6 兼容镜像均不参与模式计算，遥控链路暂时不可用时也保留当前模式。
...
- PD8 翻转子模式在存二自动、存三自动中共用：按 PD13 首段执行 `M3508=500 deg/J4310=90 deg`，按 PD12 首段执行 `M3508=0 deg/J4310=90 deg`，两者都会自动打开 PE4；等待手动关闭 PE4（`0 -> 1`）后均先等待 `200 ms`，再将 M3508 回到 `0 deg`、J4310 到 `180 deg`。存二自动的闸门 M2006 保持原目标，不再回到 `0 deg`；存三自动保持原闸门回零规则。
- 存二自动关闭 PE4 后，按 PD13 或 PD12 均可确认当前首段收尾，随后 M3508 到 `500 deg`、J4310 到 `40 deg`，再等待 `1.5 s` 自动打开 PE4；`200 ms` 关闭延时内的确认键会被锁存，`1.5 s` 打开延时内的新分支按键也会被锁存并在当前子流程结束后启动。存三自动仍只接受对应的同一按键。收尾后继续保持 PD8 翻转模式。PD12 的 `M3508=850 deg/J4310=90 deg` 第二段保持原时序，不增加这些首段动作。
- 每次启动 PD13 或 PD12 翻转首段时，自动存二、自动存三均同时执行夹爪首次 PD10 目标 `55 deg`，并将下一次实际 PD10 推进到 `135 deg`。
...
- 存二自动的 PD13 分支一为：第一次按下先清除 PD12、PC0 当前流程但保留两者的已按历史，再到 `M3508=500 deg/J4310=90 deg`；按键入口已经固定先打开 PE4，随后等待手动关闭 PE4（UART3 PE4 `0 -> 1`），关闭时 M3508 立即回 `0 deg`，`500 ms` 后 J4310 到 `180 deg`，分支一结束，下一次 PD13 直接进入分支二。第一次手动关闭 PE4 前再次按 PD13，则立即进入分支二；若 PD13 是存二自动本模式第一次存放类操作，同时执行第一次 PD10 的夹爪动作（55°），并将下一次实际 PD10 推进到 135°。存三自动仍使用原有 PD13 起始联动。
...
- 存二自动的 PD11 使用 `200 ms` 窗口区分单击和双击。单击在窗口结束后到 `M3508=500 deg/J4310=90 deg` 且闸门到 `40 deg`；再等待 `1000 ms`（即首次按下约 `1200 ms`）后自动打开 PE4，同时将 M3508 回到 `0 deg`、J4310 移到 `180 deg`，流程结束且不自动关闭 PE4，并将普通 PD9 的下一次动作复位为 `180 deg`。双击在第二次按下时取消当前 PD13/PD12 自动流程，立即到 `M3508=1050 deg/J4310=90 deg` 且闸门到 `40 deg`；再等待 `1500 ms` 后自动打开 PE4并回到 `M3508=0 deg/J4310=180 deg`，流程结束。PE4 自动动作以完整辅助帧连续发送 3 次。
...
- PC0 进入第一分支时自动打开 PE4；若 PC0 是存二自动本模式第一次存放类操作，同时执行第一次 PD10 的夹爪动作（55°），并将下一次实际 PD10 推进到 135°；存三自动保留原有首次夹爪联动。存二自动或存三自动中按下 PD13、PD12、PD11，原按键功能照常执行，同时取消 PC0 当前流程并将该模式下一次 PC0 固定复位到第一分支。
- PE0或PD6当前档位使模式发生切换时，清除旧模式中未完成的流程、延时和六个按键的单双次状态，新模式从各按键第一段开始。
- 各模式单独保留 PD13、PD12、PC0 的首次按键历史用于 PE4 起始联动；离开自动模式再返回时，PD13/PD12 仍从第一段执行。存二自动中 PC0、PD13、PD12 三者第一次触发存放类操作时，夹爪按第一次 PD10 的逻辑联动到 55°（启用堵转保护），且下一次实际按 PD10 执行第二段 135°；存三自动保留原有行为。
