...
配置依据：`D:\桌面\网表文件\暑期培训\培训H723VGT6.tel`。
...
工程使用本地 `Middlewares/Third_Party/FreeRTOS`，Keil 工程没有指向上级 `Common`
或其他外部源码目录。ST 的 H7 V1.13.0 配置明确对 H7 使用 `GCC/ARM_CM4F` FreeRTOS
portable layer；这是该固件包的官方配置，不是 MCU 被识别成 Cortex-M4。

在其他电脑编译需要 Keil ArmClang 6 和对应 STM32H7xx DFP。发送时应发送完整
`H723VGT6` 文件夹，而不是只发送 `MDK-ARM`。
