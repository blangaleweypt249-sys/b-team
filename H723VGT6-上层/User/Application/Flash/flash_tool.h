...
/**
 * @brief Initialize the W25Qxx port and use the ordinary UART command channel
 *        as the Flash text console.
 */
void FlashTool_Init(void);

/**
 * @brief Send the startup status through the default UART4 channel.
 * @param comm_ready true when the communication runtime initialized normally.
 */
void FlashTool_SendReady(bool comm_ready);

/**
 * @brief Route ordinary UART data to the Flash text protocol when it is active.
...
/**
 * @brief Return whether an ordinary UART is currently owned by the Flash text tool.
 */
