                                     ...
                                     upper_target_t *target)
{
    ...
    target->conveyor.enabled =
        (enable_mask & UPPER_ENABLE_CONVEYOR) != 0U;
    ...
    target->conveyor.pid_update = extended && target->conveyor.enabled;
                              ...
                              size_t output_size)
{
    ...
    UpperPcLink_WriteFloat(&payload[20], j4310_position_rad);
    payload[24] = j4310_position_valid ? 1U : 0U;
    UpperPcLink_WriteU32(&payload[25], j4310_bus_rx_frames);
    if (j4310_rx != NULL)
    {
