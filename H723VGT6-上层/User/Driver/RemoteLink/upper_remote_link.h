...
#define UPPER_REMOTE_LINK_H /**< 防止 upper_remote_link.h 被重复包含。 */

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>

#define UPPER_REMOTE_TIMEOUT_MS       200U /**< 超过该时间未收到遥控帧后判定遥控链路离线，单位：毫秒。 */
#define UPPER_REMOTE_PRIMARY_KEY_MASK    0x06U /**< 主遥控PC0、PC1按键位掩码。 */
#define UPPER_REMOTE_PRIMARY_SWITCH_MASK 0x03U /**< 主遥控PE0、PD6开关位掩码。 */
#define UPPER_REMOTE_KEY_MASK         0x3FU /**< 副遥控PD8至PD13按键位掩码。 */
#define UPPER_REMOTE_SWITCH_MASK      0x37U /**< 副遥控PD6、PE4开关位掩码。 */
#define UPPER_REMOTE_FRAME_BUFFER_SIZE 32U /**< 遥控接收帧缓存的字节容量。 */

#define UPPER_REMOTE_PRIMARY_KEY_PC0     (1U << 2U) /**< 主遥控PC0开关标志位。 */
#define UPPER_REMOTE_PRIMARY_KEY_PC1     (1U << 1U) /**< 主遥控PC1开关标志位。 */
#define UPPER_REMOTE_PRIMARY_SWITCH_PE0 (1U << 0U) /**< 主遥控PE0开关标志位。 */
#define UPPER_REMOTE_PRIMARY_SWITCH_PD6 (1U << 1U) /**< 主遥控PD6开关标志位。 */
#define UPPER_REMOTE_SWITCH_PD6          (1U << 4U) /**< 副遥控PD6开关标志位。 */
#define UPPER_REMOTE_MODE_STABLE_FRAMES  3U /**< PE0、PD6 模式开关状态必须连续一致才确认切换的帧数。 */
...
typedef struct
{
    ...
    uint8_t sequence; /**< 最近一帧有效遥控数据的序号。 */
    uint32_t updated_at_ms; /**< 最近一次收到有效反馈的系统毫秒时刻。 */
    bool online; /**< 副遥控链路当前是否在线。 */
...
typedef struct
{
    ...
    size_t buffered_size; /**< 接收缓冲区内尚未完成解析的字节数。 */
