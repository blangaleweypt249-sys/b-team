...
typedef struct
{
    uint32_t active_mask; /**< 记录各对象对应状态的位图。 */
    uint32_t offline_mask; /**< 记录各对象对应状态的位图。 */
    uint32_t fault_mask; /**< 记录各对象对应状态的位图。 */
    uint32_t protocol_block_mask; /**< 记录各对象对应状态的位图。 */
...
typedef struct
{
    ...
    uint8_t error_code; /**< 需要上报的故障码。 */
    uint32_t tick_ms; /**< 记录该事件时的系统毫秒时刻。 */
    uint32_t sequence; /**< 用于匹配请求和响应的消息序号。 */
} upper_motor_fault_t;
...
typedef struct
{
    ...
    float rotor_position_rad; /**< 模块的当前位置，单位：弧度。 */
    float zero_rotor_position_rad; /**< 模块的零点位置，单位：弧度。 */
    float relative_output_position_rad; /**< 模块的当前位置，单位：弧度。 */
...
/** 电机反馈超时事件上报给上位机时使用的错误码。 */
#define UPPER_MOTOR_ERROR_FEEDBACK_TIMEOUT 0xF0U

/* 功能：初始化电机协议驱动、拓扑状态和故障状态；用途：建立上层统一电机端口；返回 true 表示全部型号初始化成功。 */
bool UpperMotorPort_Init(const motor_cfg_t *cfg /* 初始化或更新时使用的配置参数 */, size_t motor_count /* 调用方提供的电机配置数量 */);
...
bool UpperMotorPort_Send(const motor_cfg_t *cfg /* 初始化或更新时使用的配置参数 */,
...
bool UpperMotorPort_GetHealth(uint32_t tick_ms /* 当前系统毫秒时刻 */,
                               upper_motor_health_t *health /* 函数读取或写入的对象地址 */);
...
bool UpperMotorPort_GetJ4310Feedback(uint8_t can_bus /* CAN 总线编号 */,
                                     uint8_t node_id /* 电机协议节点编号 */,
                                     upper_j4310_feedback_t *feedback /* 用于写出或读取最新反馈的对象 */);
/* 读取机构坐标系下的新鲜 M2006 位置、速度和电流。 */
bool UpperMotorPort_GetM2006Feedback(uint8_t can_bus /* CAN 总线编号 */,
                                     uint8_t node_id /* 电机协议节点编号 */,
                                     upper_m2006_feedback_t *feedback /* 用于写出或读取最新反馈的对象 */);
...
bool UpperMotorPort_IsDjiConfigured(uint8_t can_bus /* CAN 总线编号 */,
                                    motor_model_t model /* 需要匹配或上报的电机型号 */,
                                    uint8_t node_id /* 电机协议节点编号 */);
