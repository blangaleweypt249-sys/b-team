...
typedef enum
{
    M2006_MODE_STOP, /**< 停止输出。 */
    M2006_MODE_CURRENT, /**< 按目标电流控制。 */
    M2006_MODE_VELOCITY, /**< 按目标速度闭环控制。 */
    M2006_MODE_POSITION /**< 按目标位置闭环控制。 */
} m2006_mode_t;

/** 保存 M2006 初始化和控制所需的配置参数。 */
typedef struct
{
    float kp; /**< 比例增益。 */
    float ki; /**< 积分增益。 */
...
typedef struct
{
    float final_position_rad; /**< M2006的目标位置，单位：弧度。 */
    float reference_position_rad; /**< M2006的轨迹参考位置，单位：弧度。 */
    float final_velocity_rad_s; /**< 当前轨迹结束时要求达到的目标速度，单位：弧度每秒。 */
    float reference_velocity_rad_s; /**< M2006的轨迹参考速度，单位：弧度每秒。 */
    float reference_acceleration_rad_s2; /**< M2006的轨迹加速度，单位：弧度每二次方秒。 */
    float current_command_a; /**< M2006的目标电流，单位：安培。 */
    float position_error_rad; /**< M2006的位置误差，单位：弧度。 */
    float velocity_error_rad_s; /**< M2006的速度误差，单位：弧度每秒。 */
...
typedef enum
{
    M2006_AUTOTUNE_IDLE, /**< 当前未执行该流程。 */
    M2006_AUTOTUNE_RUNNING, /**< 当前正在执行该流程。 */
    M2006_AUTOTUNE_COMPLETE, /**< 流程已经成功完成。 */
    M2006_AUTOTUNE_FAILED /**< 流程因条件不满足或超时而失败。 */
...
typedef struct
{
    ...
    m2006_pid_cfg_t result; /**< 自动整定完成后计算得到的速度环 PID 参数。 */
...
typedef struct
{
    ...
    float output_pos_rad; /**< M2006的当前位置，单位：弧度。 */
    float output_vel_rad_s; /**< M2006的当前速度，单位：弧度每秒。 */
