...
#define M2006_ENCODER_COUNTS   8192U /**< M2006 输出轴转子编码器每圈的计数值。 */
#define M2006_REDUCTION_RATIO  36.0f /**< M2006 输出轴转子到机构输出轴的减速比。 */
#define M2006_CURRENT_RAW_MAX  10000 /**< M2006 输出轴电流命令的协议原始值满量程。 */
#define M2006_CURRENT_MAX_A    10.0f /**< M2006 输出轴协议满量程对应的电流，单位：安培。 */
#define M2006_TWO_PI           6.28318530718f /**< 角度换算使用的二倍圆周率数值。 */
#define M2006_QUINTIC_MAX_SPEED 1.875f /**< 五次平滑插值曲线的一阶导数最大系数，用于根据行程和速度计算轨迹时长。 */
#define M2006_QUINTIC_MAX_ACCEL 5.7735026919f /**< 五次平滑插值曲线的二阶导数最大系数，用于根据行程和加速度计算轨迹时长。 */
#define M2006_MIN_TRAJECTORY_S  0.01f /**< M2006 输出轴平滑位置轨迹允许使用的最短时间，单位：秒。 */
#define M2006_AUTOTUNE_TIMEOUT_MS 15000U /**< M2006 速度环自动整定允许持续的最长时间，单位：毫秒。 */
#define M2006_AUTOTUNE_CYCLES   5U /**< M2006 输出轴速度环自动整定需要采集的完整振荡周期数。 */
#define M2006_ZERO_STABLE_FRAMES 5U /**< M2006 输出轴建立软件零点前编码器必须连续稳定的反馈帧数。 */
#define M2006_ZERO_MAX_STEP_COUNTS 128 /**< M2006 输出轴置零期间相邻反馈仍可视为静止的最大编码器计数差。 */
#define M2006_ZERO_MAX_SPEED_RPM 30 /**< M2006 输出轴允许建立软件零点的最大转子速度，单位：转每分。 */

/** 保存 M2006 运行过程中需要集中管理的数据。 */
typedef struct
{
    m2006_pid_cfg_t cfg; /**< M2006 单个 PID 控制环的增益和限幅配置。 */
...
typedef struct
{
    m2006_autotune_state_t state; /**< M2006 速度环自动整定的对外状态。 */
...
typedef struct
{
    ...
    float speed_ramp_start_rad_s; /**< M2006 速度斜坡启动时的参考速度，单位：弧度每秒。 */
    ...
    bool speed_reference_valid; /**< M2006 速度斜坡参考值是否已经初始化。 */
    ...
    m2006_pid_t speed_pid; /**< M2006 速度环 PID 运行状态。 */
    m2006_pid_cfg_t speed_pid_base; /**< M2006 速度环在线调参前的基础 PID 配置。 */
    motor_online_pid_t speed_online_pid; /**< M2006 速度环在线 PID 调参状态。 */
    m2006_pid_t position_pid; /**< M2006 位置环 PID 运行状态。 */
...
static bool M2006_IsFinite(float value /**< 待检查或限幅的 M2006 控制量 */)
{
...
static bool M2006_IsValidPid(const m2006_pid_cfg_t *cfg /**< 待设置或校验的 M2006 PID 配置 */)
{
...
static float M2006_Clamp(float value /**< 待检查或限幅的 M2006 控制量 */, float min /**< 允许输出的下限 */, float max /**< 允许输出的上限 */)
{
    if (value < min)
    {
        return min;
...
static uint16_t M2006_ReadU16Be(const uint8_t *data /**< M2006反馈帧中大端16位字段的首地址 */)
{
...
static bool M2006_ConfigureOnlinePid(m2006_context_t *context /**< 需要更新的 M2006 驱动上下文 */,
...
static float M2006_SpeedPidCalc(m2006_context_t *context /**< 需要更新的 M2006 驱动上下文 */,
                                float error /**< 当前控制误差 */,
                                float dt_s /**< 本次计算的控制周期，单位：秒 */,
                                float output_limit /**< PID 控制器输出的绝对值上限 */)
{
...
static void M2006_ResetControl(m2006_context_t *context /**< 需要更新的 M2006 驱动上下文 */)
{
...
static void M2006_BeginPositionTrajectory(m2006_context_t *context /**< 需要更新的 M2006 驱动上下文 */,
...
static void M2006_UpdatePositionTrajectory(m2006_context_t *context /**< 需要更新的 M2006 驱动上下文 */,
                                            float dt_s /**< 本次计算的控制周期，单位：秒 */)
{
    float tau;
    float tau2;
...
static void M2006_UpdateSpeedRamp(m2006_context_t *context /**< 需要更新的 M2006 驱动上下文 */,
...
static void M2006_FailAutoTune(m2006_context_t *context /**< 需要更新的 M2006 驱动上下文 */)
{
...
static float M2006_AutoTuneStep(m2006_context_t *context /**< 需要更新的 M2006 驱动上下文 */,
                                float velocity_rad_s /**< M2006 当前实测速度，单位：弧度每秒 */,
...
static void M2006_UpdateTimeoutStats(m2006_context_t *context /**< 需要更新的 M2006 驱动上下文 */,
                                     bool feedback_valid /**< 当前反馈快照是否有效 */,
                                     const m2006_feedback_t *feedback /**< 本周期用于更新 M2006 超时统计的反馈快照 */,
                                     uint32_t tick_ms /**< 当前系统毫秒时刻 */)
{
