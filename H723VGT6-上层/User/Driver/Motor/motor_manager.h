...
typedef bool (*motor_send_t)(const motor_cfg_t *cfg /* 初始化或更新时使用的配置参数 */,
...
typedef struct
{
    const motor_cfg_t *cfg; /**< 电机当前使用的配置参数。 */
    size_t motor_count; /**< 当前配置或会话实际包含的电机数量。 */
    motor_cmd_t cmd[MOTOR_MANAGER_MAX_COUNT]; /**< 各电机在正常控制路径中等待发送的命令。 */
    bool enabled[MOTOR_MANAGER_MAX_COUNT]; /**< 对应控制功能是否启用。 */
    ...
    motor_send_t send; /**< 处理对应事件时调用的回调函数。 */
...
bool MotorManager_Init(motor_manager_t *manager /* 需要操作的电机管理器 */,
                       const motor_cfg_t *cfg /* 初始化或更新时使用的配置参数 */,
