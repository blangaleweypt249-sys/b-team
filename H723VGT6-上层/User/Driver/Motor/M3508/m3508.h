...
/** 该模块可同时管理的机械臂 M3508 输出轴数量。 */
#define M3508_MOTOR_COUNT    8U
/** 机械臂 M3508 输出轴默认使用的加速度上限，单位：弧度每二次方秒。 */
...
typedef struct
{
    m3508_autotune_status_t status; /**< 当前处理状态。 */
...
typedef struct
{
    ...
    m3508_pid_cfg_t speed_pid; /**< M3508对应控制环的 PID 参数或运行状态。 */
    m3508_pid_cfg_t position_pid; /**< M3508对应控制环的 PID 参数或运行状态。 */
} m3508_cfg_t;
...
bool M3508_GetFeedback(uint8_t can_bus /* CAN 总线编号 */,
                       uint8_t motor_id /* DJI 电机编号 */,
                       m3508_feedback_t *feedback /* 用于写出或读取最新反馈的对象 */);
...
bool M3508_GetAutoTuneState(uint8_t can_bus /* CAN 总线编号 */,
                            uint8_t motor_id /* DJI 电机编号 */,
                            m3508_autotune_state_t *state /* 需要检查或上报的当前状态 */);
