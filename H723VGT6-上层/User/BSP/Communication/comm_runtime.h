...
/** 上电后默认承载上位机协议的 UART 通道。 */
#define COMM_UART_PC COMM_UART_UART4

typedef void (*comm_uart_handler_t)(comm_uart_channel_t channel /* 需要选择或上报的 UART 通道 */,
...
void CommRuntime_SetHandlers(comm_uart_handler_t uart_handler /* 对应事件发生时调用的回调函数 */,
                             comm_can_handler_t can_handler /* 对应事件发生时调用的回调函数 */,
                             void *user_data /* 调用回调函数时传递的用户上下文 */);
...
bool CommRuntime_PcTransmit(const uint8_t *data /* 待处理数据的首地址 */, uint16_t size /* 待处理数据的字节数 */);
/* 功能：复制并异步发送启动或诊断消息；用途：支持栈上数据且不阻塞任务。 */
bool CommRuntime_PcTransmitCopy(const uint8_t *data /* 待处理数据的首地址 */, uint16_t size /* 待处理数据的字节数 */);
/* 功能：通过 UART5 发送辅助控制帧；用途：向抬升 H723 转发辅助输出状态。 */
bool CommRuntime_AuxUartTransmit(const uint8_t *data /* 待处理数据的首地址 */, uint16_t size /* 待处理数据的字节数 */);
...
void CommRuntime_SetPcChannel(comm_uart_channel_t channel /* 需要选择或上报的 UART 通道 */);
...
bool CommRuntime_TelemetryTransmit(const uint8_t *data /* 待处理数据的首地址 */, uint16_t size /* 待处理数据的字节数 */);
...
void DmaCache_PrepareTx(const void *buffer /* 函数读取或写入的对象地址 */, size_t size /* 待处理数据的字节数 */);
/* 功能：接收 DMA 前清理并失效对应 D-Cache；用途：避免脏缓存覆盖外设写入；无返回值表示缓冲区已准备。 */
void DmaCache_PrepareRx(void *buffer /* 函数读取或写入的对象地址 */, size_t size /* 待处理数据的字节数 */);
/* 功能：接收 DMA 后失效对应 D-Cache；用途：让 CPU 读取外设写入的最新数据；无返回值表示缓存已同步。 */
void DmaCache_CompleteRx(void *buffer /* 函数读取或写入的对象地址 */, size_t size /* 待处理数据的字节数 */);
