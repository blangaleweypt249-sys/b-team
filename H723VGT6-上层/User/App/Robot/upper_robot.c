                     ...
                     void *user_data)
{
    ...
    robot->target.gate.position_mode = true;
    robot->target.gate.enabled = true;
...
void UpperRobot_EStop(upper_robot_t *robot)
{
    ...
    robot->target.gate.enabled = false;
...
void UpperRobot_Control1ms(upper_robot_t *robot, uint32_t tick_ms)
{
    ...
    if (!Gate_Calc(&robot->target.gate, &gate_output) ||
        !Gate_Apply(&robot->motor_manager, &gate_output))
    {
        UpperRobot_DisableGate(&robot->motor_manager);
    }
