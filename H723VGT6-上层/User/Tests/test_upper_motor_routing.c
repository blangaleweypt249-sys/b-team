...
#define TEST_TX_CAPACITY  16U /**< 电机路由测试发送记录数组的容量。 */
#define TEST_DJI_ZERO_STABLE_FRAMES 5U /**< 电机建立软件零点前编码器必须连续稳定的反馈帧数。 */
#define TEST_PI 3.14159265359f /**< 角度换算使用的圆周率数值。 */
#define TEST_TWO_PI 6.28318530718f /**< 角度换算使用的二倍圆周率数值。 */
#define TEST_CONTROL_PERIOD_MS 1U /**< 测试中模拟电机控制任务的执行周期，单位：毫秒。 */
#define TEST_J4310_POSITION_MAX_RAD 12.5f /**< 机械臂 J4310 关节允许接收的最大位置目标，单位：弧度。 */
#define TEST_J4310_DIRECTION_SIGN (-1.0f) /**< 机械臂 J4310 关节从电机坐标系换算到机构坐标系时使用的方向系数。 */
#define TEST_J4310_TORQUE_MAP_MAX_NM 10.0f /**< 测试 J4310 MIT 转矩字段编码时使用的满量程，单位：牛米。 */
#define TEST_M3508_1_DIRECTION_SIGN 1.0f /**< 第一台机械臂 M3508 输出轴从电机坐标系换算到机构坐标系时使用的方向系数。 */
#define TEST_M3508_2_DIRECTION_SIGN (-1.0f) /**< 第二台机械臂 M3508 输出轴从电机坐标系换算到机构坐标系时使用的方向系数。 */
#define TEST_M3508_POSITION_VEL_LIMIT_RAD_S 15.708f /**< 机械臂 M3508 输出轴执行位置轨迹时允许的最大速度，单位：弧度每秒。 */
#define TEST_M3508_ACCEL_LIMIT_RAD_S2 62.832f /**< 机械臂 M3508 输出轴轨迹规划使用的最大加速度，单位：弧度每二次方秒。 */
#define TEST_M2006_POSITION_CUTOFF_RAD 6.45771823238f /**< M2006 输出轴的位置积分分离开始生效的误差阈值，单位：弧度。 */
#define TEST_GRIPPER_M2006_POSITION_CUTOFF_RAD 12.74090353956f /**< 夹爪机构的位置积分分离开始生效的误差阈值，单位：弧度。 */
...
typedef struct
{
    uint8_t can_bus; /**< 测试捕获的发送帧所在 CAN 总线编号。 */
...
static int16_t Test_ReadI16Be(const uint8_t *data /**< 测试CAN帧中大端16位字段的首地址 */)
{
    return (int16_t)(((uint16_t)data[0] << 8U) | data[1]);
}

/* 功能：从测试帧读取小端单精度浮点数；用途：核对 J4310 位置速度帧；返回值表示解码结果。 */
static float Test_ReadFloatLe(const uint8_t *data /**< 测试CAN帧中小端浮点字段的首地址 */)
{
    uint32_t raw;
    float value;
...
static uint16_t Test_ReadJ4310TorqueRaw(const can_frame_t *frame /**< 待解析的 CAN 接收帧 */)
{
...
static float Test_ReadJ4310MitPosition(const can_frame_t *frame /**< 待解析的 CAN 接收帧 */)
{
    uint16_t raw;
...
static void Test_FeedJ4310State(uint32_t tick_ms /**< 当前系统毫秒时刻 */, uint8_t state /**< J4310 反馈帧中的电机状态码 */)
{
    can_frame_t frame = {0};
...
static void Test_FeedJ4310Position(float mechanism_position_rad /**< 测试注入的 J4310 机构关节角，单位：弧度 */,
                                   uint32_t tick_ms /**< 当前系统毫秒时刻 */,
                                   uint8_t state /**< J4310 反馈帧中的电机状态码 */)
{
    can_frame_t frame = {0};
...
static bool Test_IsJ4310Special(const can_frame_t *frame /**< 待解析的 CAN 接收帧 */, uint8_t command /**< 期望匹配的 J4310 特殊命令码 */)
{
    size_t index;
...
static void Test_CheckJ4310ReferenceModes(motor_cmd_t *command /**< 用于依次测试各 J4310 控制模式的电机命令 */,
                                           uint32_t *tick_ms /**< 当前系统毫秒时刻 */)
{
