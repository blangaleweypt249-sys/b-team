...
def draw_side(ax, side, labels, show_note):
    start = START[side]
    ...
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_title(labels["title_normal" if side == "normal"
                 else "title_mirrored"] + "   " + labels["field_size"],
...
def main():
    ...
    for side in sides:
        ...
        ax.legend(handles=handles, loc="upper left", fontsize=9,
                  framealpha=0.9)
        fig.tight_layout()
        out_path = os.path.join(args.outdir, f"map_{side}.png")
