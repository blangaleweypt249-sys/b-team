...
static float Path_CommandYawDeg(void)
{
    ...
    return path_diagnostics.return_mode ? PATH_MAP_RETURN_YAW_DEG
...
static bool Path_UpdateOdometryAndRoute(void)
{
    ...
    {
        path_map_origin_x_m = path_diagnostics.initial_map_x_m -
                              odometry.fused_position_x_m;
        path_map_origin_y_m = path_diagnostics.initial_map_y_m -
                              odometry.fused_position_y_m;
        path_diagnostics.map_x_m = path_diagnostics.initial_map_x_m;
        path_diagnostics.map_y_m = path_diagnostics.initial_map_y_m;
...
static bool Path_ApplyMapLimit(int16_t *vx, int16_t *vy)
{
    ...
    Path_RotateVector((float)*vx / magnitude, (float)*vy / magnitude,
                      Path_CommandYawDeg(), &map_dir_x, &map_dir_y);
...
static bool Path_ApplyLaserLimits(int16_t *vx, int16_t *vy)
{
    ...
    float distance_m;
    bool hard_stop = false;
    bool returning = path_diagnostics.return_mode;
    ...
    /*
     * 左光门控：常规侧去程面向西墙，20 cm 饱和仍按“墙在量程边界”
     * 保守限速；其余情况（镜像侧，或回程时左光转向东墙/空旷场地）
     * 只有出现真实目标（< 20 cm，例如对抗中靠近的另一台车）才启用
     * 左光限速。
     */
    if (path_diagnostics.left_laser_online &&
        ((path_diagnostics.left_distance_cm < PATH_LASER_MAX_CM) ||
         (!returning && !PathMap_IsMirrored())))
    {
...
void Path_Run1ms(uint32_t now_ms)
{
    ...
    /*
     * 掉头旋转期间（回程模式且 yaw 未对齐目标角）忽略平移命令，
