...
/*
 * 回程：车头朝 -Y（yaw 锁 180°），路线为去程的逆序反向，
 * 从去程终点回到起点。X 段方向与去程相反，Y 段全部为 -Y。
 * 常规侧终点 (0.5, 3.7) -> 起点 (0.374, 0.3085)；
 * 最后两段回到初始锚定位置，path.c 在走完后强制归零。
 */
static const path_map_route_segment_t
path_map_return_route[PATH_MAP_ROUTE_SEGMENT_COUNT] =
{
    {PATH_MAP_AXIS_X, -1, 0.360f},
    {PATH_MAP_AXIS_Y, -1, 2.600f},
    {PATH_MAP_AXIS_X,  1, 2.480f},
    {PATH_MAP_AXIS_Y, -1, 1.600f},
    {PATH_MAP_AXIS_X, -1, 0.374f},
    {PATH_MAP_AXIS_Y, -1, PATH_MAP_INITIAL_CENTER_Y_M}
};

/* 镜像侧回程：从 (2.5, 3.7) 回到 (2.626, 0.3085)。 */
static const path_map_route_segment_t
path_map_return_route_mirrored[PATH_MAP_ROUTE_SEGMENT_COUNT] =
{
    {PATH_MAP_AXIS_X,  1, 2.640f},
    {PATH_MAP_AXIS_Y, -1, 2.600f},
    {PATH_MAP_AXIS_X, -1, 0.520f},
    {PATH_MAP_AXIS_Y, -1, 1.600f},
    {PATH_MAP_AXIS_X,  1, PATH_MAP_MIRRORED_START_X_M},
    {PATH_MAP_AXIS_Y, -1, PATH_MAP_INITIAL_CENTER_Y_M}
};

static bool path_map_mirrored;
static bool path_map_returning;
static bool path_map_final_approach;
...
const path_map_route_segment_t *PathMap_GetReturnRoute(uint8_t *count)
{
    ...
    return path_map_mirrored ? path_map_return_route_mirrored
                             : path_map_return_route;
}
...
{
    path_map_returning = returning;
}

bool PathMap_IsReturnMode(void)
{
    return path_map_returning;
}

void PathMap_SetFinalApproach(bool enabled)
{
    path_map_final_approach = enabled;
}

bool PathMap_IsFinalApproach(void)
{
    return path_map_final_approach;
}
                            ...
                            float map_y_m)
{
    ...
    if (path_map_returning)
    {
        segment = path_map_mirrored ?
                  &path_map_return_route_mirrored[segment_index] :
                  &path_map_return_route[segment_index];
    }
    else
    {
                           ...
                           float dir_x, float dir_y)
{
    ...
    {
        /* 回程末段贴靠起点时南向安全线收窄，允许回到起点中心 Y。 */
        float south_margin_m = path_map_final_approach ?
                               PATH_MAP_FINAL_APPROACH_MARGIN_Y_M :
                               PATH_MAP_BOUNDARY_MARGIN_Y_M;

        candidate = (south_margin_m - map_y_m) / dir_y;
        clearance = PathMap_MinFloat(clearance,
                                     (candidate > 0.0f) ? candidate : 0.0f);
    }
