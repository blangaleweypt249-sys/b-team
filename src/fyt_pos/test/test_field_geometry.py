...
def test_maps_each_bottom_start_to_its_local_first_quadrant():
    # 车体尺寸 0.70m(长) x 0.56m(宽), 车体中心初始位置 = (车宽/2, 车长/2) = (0.28, 0.35)
    # 蓝方 bottom_left: 坐标不变 + 车体半宽半长偏移
    left_position = start_corner_transform(
        Point2D(2.0, 0.0), 'bottom_left', 0.70, 0.56
    )
    assert left_position.x == pytest.approx(2.28)  # 2.0 + 0.28
    assert left_position.y == pytest.approx(0.35)  # 0.0 + 0.35

    # 红方 bottom_right: 翻转 X 轴 + 车体半宽半长偏移
    right_position = start_corner_transform(
        Point2D(2.0, 1.0), 'bottom_right', 0.70, 0.56
    )
    assert right_position.x == pytest.approx(-1.72)  # -2.0 + 0.28
    assert right_position.y == pytest.approx(1.35)  # 1.0 + 0.35