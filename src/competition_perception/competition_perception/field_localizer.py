...
from competition_perception.field_geometry import AxisAlignedZone, Point2D, classify_zone
...
class FieldLocalizer(Node):
    ...
    def _odometry_callback(self, message: Odometry) -> None:
        ...
        # 将 LIO 世界坐标平移、旋转至约定的比赛场地坐标系。
        delta_x = position.x - self._origin.x
        delta_y = position.y - self._origin.y
        heading = self._origin_yaw + self._yaw_offset
        field_x = cos(heading) * delta_x + sin(heading) * delta_y
        field_y = -sin(heading) * delta_x + cos(heading) * delta_y
        pose = PoseStamped()
